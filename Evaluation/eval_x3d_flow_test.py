"""
eval_x3d_flow_test.py

Evaluate the trained flow-based X3D-S model on the TEST split
...
with torch.no_grad():
    for xb, yb in test_loader:
        # dataset returns [B, T, 2, H, W] -> model wants [B, 2, T, H, W]
...
print(f"[OK] Flow TEST acc={test_acc:.4f} | loss={test_loss:.4f}")
...
report_path = RUNS_DIR / "classification_report_flow_TEST.txt"
...
print(f"[INFO] Saved TEST classification report to {report_path}")
...
plt.title("Confusion Matrix — Flow model (TEST)")
plt.colorbar()
...
out_path = RUNS_DIR / "confusion_matrix_flow_TEST.png"
plt.savefig(out_path, dpi=200)
print(f"Confusion matrix (TEST) saved to {out_path}")
plt.close()
